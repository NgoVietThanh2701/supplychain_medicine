...
const OrderedModal = ({ setIsOpenModal, web3Provider, uid, name, priceAgent, quantity, distance, location, image }: any) => {
   ...
   const handleBuyProduct = async () => { //open modal type address for customer
      try {
         ...
         listenEvent();
         await agtContract.approve(supplychainContract._contractAddress, priceAgent + feeShip);
      ...
      <div className='bg-half-transparent nav-item w-screen fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center'>
         <div className='bg-white w-[728px] relative group rounded-md py-5 px-6 flex flex-col gap-3 mb-16 text-333'>
            ...
            <div className='flex items-center'>
               ...
               <span className='text-444 w-[100px] text-sm'>{priceAgent} AGT</span>
            ...
            <div className='flex '>
               ...
               <div className='flex-auto flex flex-col items-end gap-1 text-[15px] text-666'>
                  <div className='flex items-center'>
                     ...
                     <span className='w-32'>{priceAgent} AGT</span>
                  </div>
                  <div className='flex items-center'>
                  ...
                  <div className='flex items-center border-t border-color pt-1'>
                     ...
                     <span className='w-36 text-[#EF4D2D] font-medium text-xl'>{feeShip + priceAgent} AGT</span>
                  </div>
               </div>
            </div>
...
const DesProduct = ({ product, onConnectMetamask, address, web3Provider }: any) => {
   ...
   const [infoFactory, SetInfoFactory] = useState<any>();
   const [infoAgent, SetInfoAgent] = useState<any>();

   const getInfoFactory = async () => {
      try {
         const responst = await apiGetInfoUser(product?.factory?.factoryCode);
         SetInfoFactory(responst.data.data);
      } catch (error: any) {
         console.log(error)
      }
   }
   ...
   const handleOpenModal = async () => {
      ...
      await caculatorFeeShip();
      setIsOpenModal(true);
   }

   const caculatorFeeShip = async () => {
      if (!payload.location) {
         Swal.fire('Opps', 'Vui lòng nhập địa chỉ nhận hàng', 'error');
      }
      ...
      <>
         {product ?
            <div className='w-5/6 mt-20'>
               {isOpenModal && <OrderedModal setIsOpenModal={setIsOpenModal} web3Provider={web3Provider} uid={product?.uid} name={product?.name} priceAgent={product?.priceAgent} quantity={product?.quantity} distance={distance} location={payload.location} image={product?.images} />}
               <div className='flex gap-7'>
                  ...
                  <div className='w-3/5 flex flex-col pt-1 gap-4'>
                     ...
                     <p className='text-444 font-medium text-xl'>Giá:  {product?.priceAgent} AGT</p>
               ...
               <div className='mt-10 flex bg-[#F5F5F5] py-3 px-5 gap-5 rounded-md'>
                  ...
                  <div className='flex flex-col justify-around'>
                     <h3 className='text-333'>Người bán: {product?.agent[0]}</h3>
               ...
               <div className='flex mt-5 gap-1'>
                  <div className='w-2/5 flex flex-col gap-4 pr-5'>
                     ...
                     <div className='flex items-center'>
                        <span className='w-44'>Hạn sử dụng</span>
                        <p>{product?.hsd}</p>
                     </div>
                     <div className='flex items-center'>
                        ...
                        >
                           ...
                           >
                              <InfoBox options={options} >
                                 <div style={{ backgroundColor: 'green', color: 'white', borderRadius: '4px', fontSize: '13px', padding: '2px 3px', textAlign: 'center' }}> Agent </div>
                              </InfoBox>
                           </MarkerF>
                           <MarkerF
                              position={{
