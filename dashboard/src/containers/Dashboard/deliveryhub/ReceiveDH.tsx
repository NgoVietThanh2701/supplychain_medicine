...
const ReceiveDH = () => {
   ...
   const getProductsShipByTPT = async () => {
      try {
         ...
         const productFilted = response.filter((data: any) => data.productState === StateProduct.ShippedByAgent);
         const listProducts = [];
   ...
   const handleConfirm = async (uid: number) => {
      ...
      try {
         ...
         listenEventConfirm();
         await supplychainContract.receiveByDeliveryHub(uid, currentUser?.code);
      } catch (error) {
         setIsLoading(false);
         console.log(error);
      }
