...
const ModalViewProduct = ({ setIsOpenModal, product }: any) => {
   ...
   const getInfoFactory = async (code: string) => {
      try {
   ...
   const getInfoAgent = async (code: string) => {
      try {
         const response = await apiGetInfoUser(code);
         setInfoAgent(response.data.data);
      } catch (error: any) {
         console.log(error)
      }
   }
   ...
   const getInfoDelivery = async (code: string) => {
      try {
         const response = await apiGetInfoUser(code);
         setInfoDelivery(response.data.data);
      } catch (error: any) {
         console.log(error)
      }
   }
   ...
   useEffect(() => {
      if (product) {
         getInfoFactory(product.factory.factoryCode);
         getInfoAgent(product.agent.agentCode);
               ...
               <div className='bg-gray-100 w-1/4 flex'>
                  <div className='relative'>
                     ...
                     <div className='flex flex-col gap-0.5 mt-2 px-2'>
                        ...
                        <span className='text-333 text-[15px]'>{infoFactory.name}</span>
                        <span className='text-333 text-sm'>({showShortAddress(infoFactory.addressWallet, 5)})</span>
                     </div>
                     <div className='relative'>
                        ...
                        <div className='flex flex-col gap-0.5 mt-20 px-2'>
                           ...
                           <span className='text-333 text-[15px]'>{infoDelivery.name}</span>
                           <span className='text-333 text-sm'>({showShortAddress(infoDelivery.addressWallet, 5)})</span>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className='relative'>
                     ...
                     <div className='flex flex-col gap-0.5 mt-24 px-1 pl-4'>
                        ...
                        <span className='text-333 text-[15px]'>{infoAgent.name}</span>
                        <span className='text-333 text-sm'>({showShortAddress(infoAgent.addressWallet, 5)})</span>
                     </div>
...
const PurchaseForm = () => {
   ...
   const getProductsShipByTPT = async () => {
      try {
         const supplychainContract = new SupplyChainContract();
         const response = await supplychainContract.getProducts();
         const productFilted = response.filter((data: any) => (data.productState === StateProduct.ShippedByAgent && data.customerDetails.customerCode === currentUser?.code));
         const listProducts = [];
         for (let i = 0; i < productFilted.length; i++) {
            listProducts.push(convertObjectProduct(productFilted[i]));
         }
   ...
   const convertObjectProduct = (data: any) => {
      return {
         ...
         totalPrice: formatToEth(data.customerDetails.feeShip) + formatToEth(data.productDetails.priceAgent),
